...
function formatDateTime(value) {
  ...
  if (Array.isArray(value)) {
    // LocalDateTime can come as [year, month, day, hour, minute, second, nano]
    const [y, m, d, hh = 0, mm = 0, ss = 0] = value;
    date = new Date(Number(y), Number(m) - 1, Number(d), Number(hh), Number(mm), Number(ss));
  } else if (typeof value === "object") {
    const y = Number(value.year);
    const m = Number(value.monthValue ?? value.month);
    const d = Number(value.dayOfMonth ?? value.day);
    ...
    // Handle Java LocalDateTime formats safely, including fractional seconds.
    ...
    if (m) {
      ...
      date = new Date(y, mo - 1, d, hh, mm, ss);
    } else {
      // Very tolerant fallback: extract yyyy-mm-dd and optional hh:mm:ss from any string.
      const loose = String(value).match(
      ...
      if (loose) {
        ...
        const hh = Number(loose[4] || 0);
        const mm = Number(loose[5] || 0);
        const ss = Number(loose[6] || 0);
        date = new Date(y, mo - 1, d, hh, mm, ss);
      } else {
...
function getProjectCreatedAtDisplay(project) {
  return (
    project?.created_at_display ??
    project?.createdAtDisplay ??
    project?.createdAtText ??
...
function getProjectCreatedAt(project) {
  ...
  const candidateKey = Object.keys(project).find((k) => /created/i.test(k));
  return candidateKey ? project[candidateKey] : null;
}
...
function splitMembersByRole(memberDetails, memberNames, userLookup) {
  ...
  // Fallback for older API responses that may not include role-aware member details.
...
export default function UserProjectDetails() {
      ...
      if (debugMode) {
        ...
        console.table(
          projectList.map((p) => ({
            ...
            created_at_display: p?.created_at_display,
            debug_created_at_raw: p?.debug_created_at_raw,
            debug_created_at_java_type: p?.debug_created_at_java_type
          }))
        );
      }
      try {
  ...
  const openProjectDetails = (project) => {
    ...
    navigate(`/user/project/${project.id}`, { state: { projectName: project?.name || "" } });
  };
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-gray-600">
                ...
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {getProjectCreatedAtDisplay(selectedProject) ||
                    formatDateTime(getProjectCreatedAt(selectedProject))}
                </span>
              </div>
              {debugMode && (
                <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  <div>{`debug created_at_display: ${selectedProject?.created_at_display ?? "-"}`}</div>
                  <div>{`debug created_at raw: ${String(getProjectCreatedAt(selectedProject) ?? "-")}`}</div>
                  <div>{`debug backend raw: ${selectedProject?.debug_created_at_raw ?? "-"}`}</div>
                  <div>{`debug backend type: ${selectedProject?.debug_created_at_java_type ?? "-"}`}</div>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          ...
          <section className="space-y-4">
            ...
            <InfoCard
              icon={Wrench}
              title="Engineers"
              value={memberSplit.engineers.length || 0}
              tone="amber"
            />
            <InfoCard
              icon={UsersRound}
              title="Users"
              value={memberSplit.users.length || 0}
              tone="emerald"
            />
            <InfoCard
              icon={CalendarDays}
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        ...
        <p className="mt-1 text-sm text-gray-600">Projects where you are a member.</p>
      </div>
                ...
                return (
                  <tr key={project.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {resolveDisplayName(project.managerName)}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        {getProjectCreatedAtDisplay(project) || formatDateTime(getProjectCreatedAt(project))}
                      </span>
                      {debugMode && (
                        <div className="mt-1 text-[11px] text-amber-700">
                          {`raw:${String(getProjectCreatedAt(project) ?? "-")} | disp:${project?.created_at_display ?? "-"} | braw:${project?.debug_created_at_raw ?? "-"}`}
                        </div>
                      )}
                    </td>
